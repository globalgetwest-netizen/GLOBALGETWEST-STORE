...
const GATEWAYS_BY_CURRENCY: Record<string, { id: 'stripe' | 'flutterwave' | 'grey'; label: string }[]> = {
  USD: [
    { id: 'stripe', label: 'Card (Stripe)' },
    { id: 'grey', label: 'USDC (Grey)' },
  ],
  GHS: [{ id: 'flutterwave', label: 'Card / Mobile Money (Flutterwave)' }],
...
export function CheckoutFlow({ cart }: { cart: CartSummary }) {
  ...
  const [gateway, setGateway] = useState<'stripe' | 'flutterwave' | 'grey'>('stripe');
