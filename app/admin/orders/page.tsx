...
export default async function AdminOrdersPage({
  ...
  let query = supabase
    .from('orders')
    .select('id, order_number, status, total_usd_cents, currency, shipping_country_code, created_at')
          ...
          <tbody>
            {(orders ?? []).map((o) => (
              <tr key={o.id} className="border-t border-[var(--color-border)]">
                ...
                <td className="px-4 py-2.5 text-[var(--color-ink-soft)]">{o.shipping_country_code ?? '—'}</td>
            ...
            {(!orders || orders.length === 0) && (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-[var(--color-ink-soft)]">No orders match this filter.</td></tr>
            )}
          </tbody>
        </table>
      </div>
